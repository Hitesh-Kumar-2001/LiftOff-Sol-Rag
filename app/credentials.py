...
class CredentialSource(Protocol):
    ...
    async def load_all(self) -> Iterable[ServerCredential]:
        """Every credential in the store, as it stands right now."""
        ...


def parse_credentials(raw: str, origin: str = "credential document") -> list[ServerCredential]:
    ...
    return [_to_credential(server_id, entry) for server_id, entry in document.items()]


def _to_credential(server_id: str, entry: object) -> ServerCredential:
    """Build a credential from one store record, whatever the store."""
    if not isinstance(entry, dict):
        raise CredentialConfigError(f"Credential for '{server_id}' must be an object.")

    secret_hash = entry.get("secretSha256")
    if secret_hash is None:
        secret = entry.get("secret")
        if not secret:
            raise CredentialConfigError(
                f"Credential for '{server_id}' needs a 'secret' or 'secretSha256'."
            )
        secret_hash = hash_secret(secret)

    return ServerCredential(server_id=server_id, secret_hash=str(secret_hash))
...
class FileCredentialSource:
    ...
    async def load_all(self) -> list[ServerCredential]:
        try:
            raw = self._path.read_text(encoding="utf-8")
        except OSError as exc:
            raise CredentialConfigError(f"Cannot read credential file '{self._path}': {exc}") from exc
        return parse_credentials(raw, str(self._path)) if raw.strip() else []
...
class EnvCredentialSource:
    ...
    async def load_all(self) -> list[ServerCredential]:
        ...
        raw = raw.strip()
        return parse_credentials(raw, ENV_SERVER_CREDENTIALS) if raw else []
...
class InMemoryCredentialSource:
    ...
    def __init__(self, credentials: Iterable[ServerCredential]) -> None:
        self._by_id = {c.server_id: c for c in credentials}

    async def load_all(self) -> list[ServerCredential]:
        return list(self._by_id.values())


def build_credential_source() -> CredentialSource:
    ...
    being polled because it declares a ``refresh_interval`` -- no other file
    changes.
    """
