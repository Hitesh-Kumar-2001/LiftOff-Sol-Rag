...
def resolveSubmission(
    ...
    *,
    ...
    One function rather than a copy inside each job manager. There are three
    of them now -- in-memory, Firestore, Celery -- and they have to answer
    this identically or the same request gets accepted on one deployment and
    refused on another. Three copies of the rules is three places for them to
    drift, and the drift would only show up in production.
    ...
    **The threshold must exceed the longest a job could still legitimately be
    ...
    default, and only set where something guarantees an upper bound: under
    Celery that is ``task_time_limit``, which kills a task outright. See
    ``buildJobManager``.
    """
...
class JobStore(Protocol):
    ...
    accepted: a Celery worker picks a job up in another process entirely, and
    needs the same table the API wrote to. Storage that knows nothing about
    the execution model is what the API and the worker can share.
    """
