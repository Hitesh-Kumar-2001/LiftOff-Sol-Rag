...
``DocumentProcessor`` is what ``app.jobs.job.runJob`` executes; this module owns
that contract because the contract is about documents, not about how a job is
scheduled or tracked (that's ``app.jobs.jobManager``). ``DocumentAnalyzerProcessor``
...
# unbounded memory and CPU -- a zip bomb is exactly this shape. One level
# covers every real submission seen so far: a zip of documents. Raising it
# means each extra level multiplies the worst-case work, so raise it only
# alongside a size budget, not on its own.
MAX_ARCHIVE_DEPTH = 1
...
class DocumentProcessor(Protocol):
    ...
    nothing but ``job.detail`` / ``job.metadata``; ``app.jobs.job.runJob`` owns
    ``job.status``.
    """
