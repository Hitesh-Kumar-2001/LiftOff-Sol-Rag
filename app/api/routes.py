"""Ingestion and retrieval routes: putting documents in, and looking them up.

Everything conversational moved to ``app.api.conversationRoutes`` when the
gateways arrived -- answering a question is now one shape whether it came from a
browser, WhatsApp or LINE, and it did not belong beside these.

What is left is the pair that has nothing to do with a conversation: submitting
a document for ingestion, polling that job, and searching a project's chunks
directly. They keep the original convention of taking every id in the **body**,
including the ``projectId``. That is deliberately not the shape the conversation
routes use: those had to put the project in the path, because a webhook body is
written by WhatsApp and nothing can be added to it. Nothing forces it here, and
changing a working contract to match a constraint that does not apply to it
would be churn.

Every endpoint takes a ``projectId``. The ``ragDbId`` behind it is resolved
here, at the edge, and everything inward -- the job manager, the conflict check,
the chunk store -- goes on keying on that id exactly as before. The indirection
stops in this file, which is what keeps it cheap: see ``app.stores.projectStore``
for why it exists at all.

**There is no authentication.** Every endpoint takes a ``serverId``, but nothing
verifies it: there is no secret, no signature, and no registry to check it
against. It is a label the caller picked, recorded in the log so a request can
be traced back to whoever says they made it, and treated as nothing more. Anyone
who can reach this API can therefore ingest into any project and read any
project's chunks, and can name themselves anything while doing it. That is a
deliberate choice for a service that is not yet exposed; putting it behind
anything public means adding authentication back first. (The webhook gateways in
``app.api.conversationRoutes`` are the one exception, and they authenticate the
*platform*, not the caller behind it.)
